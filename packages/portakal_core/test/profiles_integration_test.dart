  ...
  group('Printer profile integration', () {
    test('uses profile DPI', () {
      final resolved = label(
        LabelConfig(width: 100, height: 50, printer: 'tsc-te310'),
      ).resolve();
    ...
    test('uses profile width when not specified', () {
      ...
      final resolved = label(
        LabelConfig(width: 80, printer: 'epson-tm-t88vi'),
      ).resolve();
    ...
    test('user config overrides profile', () {
      final resolved = label(
        LabelConfig(width: 50, dpi: 600, printer: 'tsc-te200'),
      ).resolve();
    ...
    test('ignores unknown printer profile', () {
      final resolved = label(
        LabelConfig(width: 40, printer: 'unknown-model'),
      ).resolve();
