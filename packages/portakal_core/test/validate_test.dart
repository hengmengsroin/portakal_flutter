...
import 'package:portakal_core/src/validate.dart';
...
void main() {
  group('validate — TSC', () {
    ...
    test('warns when SIZE is not first', () {
      ...
      expect(
        r.issues.any((i) => i.message.contains('SIZE should be the first')),
        isTrue,
      );
    });
    ...
    test('errors when CLS missing before elements', () {
      ...
      expect(
        r.issues.any((i) => i.message.contains('CLS must appear before')),
        isTrue,
      );
    });
    ...
    test('errors on DENSITY out of range', () {
      ...
      expect(
        r.issues.any(
          (i) => i.message.contains('DENSITY value 20 out of range'),
        ),
        isTrue,
      );
    });
    ...
    test('warns on unknown commands', () {
      final r = validate('SIZE 40 mm,30 mm\nCLS\nFOOBAR\nPRINT 1', 'tsc');
      expect(
        r.issues.any((i) => i.message.contains('Unrecognized command')),
        isTrue,
      );
    });
  ...
  group('validate — ZPL', () {
    ...
    test('warns on ^FD without ^FO', () {
      ...
      expect(
        r.issues.any((i) => i.message.contains('^FD without preceding ^FO')),
        isTrue,
      );
    });
