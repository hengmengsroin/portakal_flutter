...
void main() {
  group('BarcodeSymbology & BarcodeOptions.typed Convenience API (Slice 5B Semantic Safety)', () {
    test('A. Universal BarcodeSymbology enum contains only verified universally preserved symbology (code128)', () {
      expect(BarcodeSymbology.values, equals([BarcodeSymbology.code128]));
    ...
    test('E. Cross-protocol byte equivalence for code128 across all 9 protocols', () {
      const config = LabelConfig(width: 80, height: 50);

      final legacyJob = (label(config)
            ..barcode('ITEM1234', const BarcodeOptions(x: 10, y: 20, type: '128', height: 40, readable: 1)))
          .resolve();
      final typedJob = (label(config)
            ..barcode('ITEM1234', BarcodeOptions.typed(x: 10, y: 20, symbology: BarcodeSymbology.code128, height: 40, readable: 1)))
          .resolve();
    ...
    test('F. Semantic identity proof: Code 128 emits authentic protocol-specific commands across all 9 protocols', () {
      const config = LabelConfig(width: 80, height: 50);
    ...
    test('H. Arbitrary legacy string escape hatch remains fully operational for non-universal symbologies', () {
      ...
      expect(code39Options.type, equals('39'));
