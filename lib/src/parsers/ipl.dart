...
class IPLParseResult {
  ...
  int widthDots;
  int heightDots;
  IPLParseResult({required this.commands, required this.elements, this.widthDots = 0, this.heightDots = 0});
}
...
IPLParseResult parseIPL(String code) {
  ...
  int i = 0;
  while (i < code.length) {
    if (code.codeUnitAt(i) == 0x02) { // STX
      i++;
      final start = i;
      while (i < code.length && code.codeUnitAt(i) != 0x03) i++;
  ...
  for (final frame in frames) {
    ...
    if (frame.startsWith('\x1b')) {
      ...
      if (cmd == 'C1') commands.add(IPLCommand(type: 'CREATE_FORMAT'));
      else if (cmd == 'P') commands.add(IPLCommand(type: 'PROGRAM_MODE'));
      else if (cmd == 'E1') commands.add(IPLCommand(type: 'END_FORMAT'));
      else if (cmd.startsWith('M')) commands.add(IPLCommand(type: 'MULTIPLE', params: cmd.substring(1)));
      continue;
    }
    ...
    if (frame.startsWith('<SI>')) {
      final rest = frame.substring(4);
      if (rest.startsWith('L')) {
        heightDots = int.tryParse(rest.substring(1)) ?? 0;
        commands.add(IPLCommand(type: 'LABEL_LENGTH', params: rest.substring(1)));
      } else if (rest.startsWith('W')) {
        widthDots = int.tryParse(rest.substring(1)) ?? 0;
        commands.add(IPLCommand(type: 'LABEL_WIDTH', params: rest.substring(1)));
    ...
    if (frame.startsWith('H') || frame.startsWith('W') || frame.startsWith('L')) {
      ...
      for (final part in parts) {
        ...
        if (p.startsWith('o')) fieldData['origin'] = p.substring(1);
        else if (p.startsWith('f')) fieldData['rotation'] = p.substring(1);
        else if (p.startsWith('h')) fieldData['height'] = p.substring(1);
        else if (p.startsWith('w')) fieldData['width'] = p.substring(1);
        else if (p.startsWith('l')) fieldData['length'] = p.substring(1);
        else if (p.startsWith('c')) fieldData['font'] = p.substring(1);
        else if (p.startsWith('d')) {
          final dParts = p.substring(1).split(',');
          if (dParts.length > 1) fieldData['data'] = dParts.sublist(1).join(',');
        }
      }
