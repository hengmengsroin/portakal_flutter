...
String compileToCPCL(ResolvedLabel label) {
  ...
  buf.write('! 0 ${label.dpi} ${label.dpi} ${label.heightDots} ${label.copies}\r\n');

  if (label.density > 0) {
    buf.write('TONE ${label.density > 8 ? 2 : label.density > 4 ? 1 : 0}\r\n');
  }
  if (label.speed > 0) {
  ...
  for (final el in label.elements) {
    switch (el) {
      ...
      case ImageElement():
        ...
        final y = o.y ?? 0;
        final bmp = el.bitmap;
        final hexData = bmp.data.map((b) => b.toRadixString(16).padLeft(2, '0').toUpperCase()).join();
